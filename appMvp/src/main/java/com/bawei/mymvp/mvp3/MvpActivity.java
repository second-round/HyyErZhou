...
public class MvpActivity extends AppCompatActivity implements IView, View.OnClickListener {
    ...
    private void startRequest(int type) {
        switch (type) {
            case TYPE_LOGIN:
                String strLogin = String.format(Apis.URL_LOGIN, mEtName.getText().toString(), mEtPw.getText().toString());
                mPresenter.startRequest(strLogin, null, LoginBean.class);
                break;
            ...
            case TYPE_GET_USER_INFO:
                String strGetUserInfo = String.format(Apis.URL_GET_USER_INFO, SpUtils.getString(this, Constants.SP_KEY_UID, ""));
                mPresenter.startRequest(strGetUserInfo, null, GetUserInfoBean.class);
                break;
            default:
                break;
        }
